...
* **Validación Simétrica de Parámetros:** Inicialmente, FastAPI ignoraba silenciosamente parámetros de consulta no reconocidos en la URL del GET (como typos, ej: `?CANALL=POS`). Para hacer la validación 100% estricta y simétrica con el POST, implementamos un escaneo en `get_estadisticas` que valida las llaves de `request.query_params` contra la lista blanca permitida, rechazando con un error HTTP 400 (VF) cualquier campo incorrecto.
...
1. **Pruebas Unitarias Automatizadas (con Pytest):** Son **21 pruebas** en `tests/test_api.py` que se ejecutan en segundos inyectando un dataset mock controlado de 4 filas.
2. **Pruebas Funcionales en Vivo (PowerShell):** Creamos el script `test_manual_completo.ps1` que ejecuta **37 consultas reales en vivo** contra el servidor de Uvicorn (filtros válidos, límites de edad, local en 0, floats inválidos, campos erróneos en GET, etc.) y todas pasan en verde exitosamente (`37/37 pruebas pasaron`).
